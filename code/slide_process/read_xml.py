...
def saveArray(filename,image_size,coordinates,labels):
    #red is 'benign', green is 'in situ' and blue is 'invasive'
    colors = [0,1,2,3]

    img = np.zeros(image_size[:2],dtype=np.uint8)
    print(image_size[:2])
    
    for c,l in zip(coordinates,labels):
        img1 = fillImage(img,[np.int32(np.stack(c))],color=colors[l])
        # img2 = img1[::sample,::sample,:]
    np.save(filename,img1)

if __name__=='__main__':
    
    folder_name = '/storage/ndong/data/micro/WSI/' #path to the dataset folder
    ...
    for file in files:
        ...
        saveArray(folder_name+file_name+'_gt.npy',img_size,coords,labels)